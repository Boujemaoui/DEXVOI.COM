...
import { executeAndDeliverAudit, cleanStalePendingTargets, createScopedCleanupTimer } from '../../../server/auditDelivery';
...
export async function onRequestPost(context: EventContext<CloudflareEnv, any, any>): Promise<Response> {
  ...
  // Limpieza de entradas antiguas y temporizador gestionados estrictamente DENTRO del handler
  cleanStalePendingTargets();
  const disposeTimer = createScopedCleanupTimer(60 * 60 * 1000);
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      const deliveryPromise = executeAndDeliverAudit(session, lineItems, stripe)
        .then((auditResult) => {
      ...
      if (context.waitUntil) {
        context.waitUntil(deliveryPromise);
      } else {
    ...
    disposeTimer();
    console.error('❌ [Cloudflare Pages Webhook] Excepción no controlada en handler:', globalError);
    return new Response(
      JSON.stringify({ error: 'Error interno en el procesamiento del webhook' }),
